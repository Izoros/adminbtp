...
export default async function AdminCommandsPage() {
  const result = await loadWhatsAppCommandQueue();

  if (result.access === "unauthenticated") {
    redirect(buildLoginRedirectPath("/admin/commands"));
  }

  return (
    <AppShell eyebrow="Pilotage distant securise" title="Commandes WhatsApp">
      <div className="space-y-6">
        ...
        {result.access === "ready" ? (
          <CommandQueueDashboard data={result.data} />
        ) : (
